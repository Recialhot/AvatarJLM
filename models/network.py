...
class AvatarJLM(nn.Module):
    ...
    def input_processing(self, input_tensor):
        ...
        processed_info = {}
        processed_info['global_gt_head_hand_foot_rotation'] = input_tensor[:, :, [7, 8, 15, 20, 21], 0:6].clone()
        processed_info['global_gt_head_hand_foot_position'] = input_tensor[:, :, [7, 8, 15, 20, 21], 12:15].clone()
        processed_info['global_gt_head_position'] = processed_info['global_gt_head_hand_foot_position'][:, :, 2].clone()
        processed_info['relative_gt_head_hand_foot_position'] = processed_info['global_gt_head_hand_foot_position'] - processed_info['global_gt_head_position'][:, :, None]
        # --------- set masked input ------------
        selected_joint_index = list(range(0, 6+1)) + list(range(9, 14+1))+list(range(16, 19+1))
    ...
    def random_mask(self, joint_embedding):
        ...
        for b_idx in range(batch):
            for s_idx in range(seq_len):
                selected_joint_index = list(range(0, 6+1)) + list(range(9, 14+1))+list(range(16, 19+1))
                if self.use_rotation_token:
                    selected_joint_index += list(range(0+22, 6+1+22)) + list(range(9+22, 14+1+22)) + list(range(16+22, 19+1+22))
                mask_num = 2
    ...
    def stage_1(self, input_tensor):
        ...
        # gt replace
        if self.gt_head_hand_replace:
            relative_pose_init[:, :, [7, 8, 15, 20, 21]] = processed_info['relative_gt_head_hand_foot_position']
            rotation_init[:, :, [7, 8, 15, 20, 21]] = processed_info['global_gt_head_hand_foot_rotation']
